...
def test_NIFTISampleWriter(datas, labelss, IDs, new_folder_paths):
  """
  Given the image generated by the strategy, we check that
...
def test_DataLoad(datas, labelss, IDs, new_folder_pathss):
  """
  Test the capability of DataLoad function to properly load images stored in a parent directory, 
  ...
  for i in range(len(data_paths)):
    ...
    assert (sitk.GetArrayFromImage(dataset['features'][0]) == sitk.GetArrayFromImage(data)).all()
    assert (sitk.GetArrayFromImage(dataset['labels'][0]) == sitk.GetArrayFromImage(labels)).all()
